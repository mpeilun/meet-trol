...
import { isFocusOnVideo } from '../../../../util/calculate'
            ...
            if (videoList.includes(videoId)) {
              ...
              if (pastView) {
                ...
                if (check.ownerId.includes(session.user.id)) {
                  ...
                  // await allUserData.map((user) => {
                  //   user.pastView[0].viewLogs.map((viewLog) => {
                  //     viewLog.eyesTrack.map
                  //   })
                  // })

                  const allAnalyticsLogs: Array<AnalyticsLog[]> = []
                    ...
                    for (
                    ...
                    ) {
                      {
                        ...
                        interactionList.map((interaction) => {
                          ...
                          if (interaction.id === choice1Id) {
                            ...
                            interaction.feedback.map((feedback) => {
                              ...
                              if (check(feedback.answer, answer)) {
                              ...
                              analyticsLog[
                                ...
                                isCorrect: check(feedback.answer, answer),
                              }
                            })
                          }
                          // 單選 2
                        ...
                        const dragInteractionAction: InteractionAction = {
                          ...
                          correctRate: dragCorrectTimes / rankAnswerTimes,
                        ...
                        const interactionLog: InteractionLog = {
                          ...
                          email: email,
                          name: name,
                          totalVideoWatchTime: totalVideoWatchTime,
                          totalOnVideoWatchTime: totalOnVideoWatchTime,
                          totalOutVideoWatchTime: totalOutVideoWatchTime,
                          totalVideoPauseTime: totalVideoPauseTime,
                          totalVideoFocusTime: totalVideoFocusTime,
                          ...
                          enterPostTime: leaveVideoTime.toLocaleString(),
                          interactionActions: [
                      ...
                      allAnalyticsLogs.push(analyticsLogs)
                      allInteractionLogs.push(interactionLogs)
                    }
                  }
                  return res.status(200).json(allInteractionLogs)
                } else {
